...
module.exports = { entry: { main:'./src/scripts/pages/index.js'}
, output: { path: path.resolve(__dirname, 'dist')
    , filename: 'main.js'
    , publicPath: ''
  }
, mode: 'development'
, devServer: { static: path.resolve(__dirname, './dist')        // путь, куда "смотрит" режим разработчика
    , compress: true            // это ускорит загрузку в режиме разработки
    , port: 8080                // порт, чтобы открывать сайт по адресу localhost:8080, но можно поменять порт
    , open: true                // сайт будет открываться сам при запуске npm run dev
  }
, module: { rules: [                    // rules — это массив правил; добавим в него объект правил для бабеля
